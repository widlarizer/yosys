			...
			for (auto module : design->all_selected_modules())
			{
				...
				for (auto proc : module->selected_processes())
				{
					...
					while (!all_cases.empty()) {
						...
						all_cases.pop_back();
						attrmap_apply(design, stringf("%s.%s (case)", module, log_id(proc)), actions, cs->attributes);

						for (auto &sw : cs->switches) {
							attrmap_apply(design, stringf("%s.%s (switch)", module, log_id(proc)), actions, sw->attributes);
