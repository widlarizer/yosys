	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto module : design->selected_modules()) {
			...
			for (int i = 0; i < compute_graph.size(); ++i)
			{
				...
				log("n%d ", i);
				log("%s", log_id(design, ref.function().name));
				for (auto const &param : ref.function().parameters)
				{
					if (param.second.empty())
						log("[%s]", log_id(design, param.first));
					else
						log("[%s=%s]", log_id(design, param.first), log_const(param.second));
				}
				log("(");
