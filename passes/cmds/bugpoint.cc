				...
				for (auto &pr : mod->processes)
				{
					...
					while (!cases.empty())
					{
						...
						for (auto it = cs->actions.begin(); it != cs->actions.end(); ++it)
						{
							if (index++ == seed)
							{
								log_header(design, "Trying to remove assign %s %s in %s.%s.\n", log_signal(it->first), log_signal(it->second), mod, log_id(design, pr.first));
								cs->actions.erase(it);
								return design_copy;
							}
						}
				...
				for (auto &pr : mod->processes)
				{
					for (auto &sy : pr.second->syncs)
					{
						for (auto it = sy->actions.begin(); it != sy->actions.end(); ++it)
						{
							if (index++ == seed)
							{
								log_header(design, "Trying to remove sync %s update %s %s in %s.%s.\n", log_signal(sy->signal), log_signal(it->first), log_signal(it->second), mod, log_id(design, pr.first));
								sy->actions.erase(it);
								return design_copy;
							}
						}
						...
						for (auto it = sy->mem_write_actions.begin(); it != sy->mem_write_actions.end(); ++it, ++i)
						{
							if (index++ == seed)
							{
								log_header(design, "Trying to remove sync %s memwr %s %s %s %s in %s.%s.\n", log_signal(sy->signal), log_id(design, it->memid), log_signal(it->address), log_signal(it->data), log_signal(it->enable), mod, log_id(design, pr.first));
