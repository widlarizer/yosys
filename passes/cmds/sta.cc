...
struct StaWorker
{
	...
	void run()
	{
		...
		if (it != endpoints.end() && it->second.sink)
			log("  %6d %s (%s.%s)\n", maxarrival, it->second.sink, it->second.sink->type.unescape(), log_id(it->second.sink->module, it->second.port));
		else {
		...
		while (jt != data.end()) {
			...
			if (jt->second.driver) {
				log("           %s\n", log_signal(b));
				log("  %6d %s (%s.%s->%s)\n", arrival, jt->second.driver, jt->second.driver->type.unescape(), log_id(jt->second.driver->module, jt->second.src_port), log_id(jt->second.driver->module, jt->second.dst_port));
			}
