...
private:
	...
	void add_precise_GLIFT_logic(const RTLIL::Cell *cell, RTLIL::SigSpec &port_a, RTLIL::SigSpec &port_a_taint, RTLIL::SigSpec &port_b, RTLIL::SigSpec &port_b_taint, RTLIL::SigSpec &port_y_taint) {
		...
		RTLIL::SigSpec n_port_a = module->LogicNot(Twine{cell->name.unescape() + "_t_1_1"}, port_a, false, cell->src_ref());
		RTLIL::SigSpec n_port_b = module->LogicNot(Twine{cell->name.unescape() + "_t_1_2"}, port_b, false, cell->src_ref());
		auto subexpr1 = module->And(Twine{cell->name.unescape() + "_t_1_3"}, is_and? port_a : n_port_a, port_b_taint, false, cell->src_ref());
		auto subexpr2 = module->And(Twine{cell->name.unescape() + "_t_1_4"}, is_and? port_b : n_port_b, port_a_taint, false, cell->src_ref());
		auto subexpr3 = module->And(Twine{cell->name.unescape() + "_t_1_5"}, port_a_taint, port_b_taint, false, cell->src_ref());
		auto subexpr4 = module->Or(Twine{cell->name.unescape() + "_t_1_6"}, subexpr1, subexpr2, false, cell->src_ref());
		module->addOr(Twine{cell->name.unescape() + "_t_1_7"}, subexpr4, subexpr3, port_y_taint, false, cell->src_ref());
	}
	...
	void add_imprecise_GLIFT_logic_1(const RTLIL::Cell *cell, RTLIL::SigSpec &port_a, RTLIL::SigSpec &port_a_taint, RTLIL::SigSpec &port_b, RTLIL::SigSpec &port_b_taint, RTLIL::SigSpec &port_y_taint) {
		...
		bool is_and = cell->type.in(ID($_AND_), ID($_NAND_));
		RTLIL::SigSpec n_port_a = module->LogicNot(Twine{cell->name.unescape() + "_t_2_1"}, port_a, false, cell->src_ref());
		auto subexpr1 = module->And(Twine{cell->name.unescape() + "_t_2_2"}, is_and? port_b : n_port_a, is_and? port_a_taint : port_b_taint, false, cell->src_ref());
		module->addOr(Twine{cell->name.unescape() + "_t_2_3"}, is_and? port_b_taint : port_a_taint, subexpr1, port_y_taint, false, cell->src_ref());
	}
	...
	void add_imprecise_GLIFT_logic_2(const RTLIL::Cell *cell, RTLIL::SigSpec &port_a, RTLIL::SigSpec &port_a_taint, RTLIL::SigSpec &port_b, RTLIL::SigSpec &port_b_taint, RTLIL::SigSpec &port_y_taint) {
		...
		bool is_and = cell->type.in(ID($_AND_), ID($_NAND_));
		RTLIL::SigSpec n_port_b = module->LogicNot(Twine{cell->name.unescape() + "_t_3_1"}, port_b, false, cell->src_ref());
		auto subexpr1 = module->And(Twine{cell->name.unescape() + "_t_3_2"}, is_and? port_a : n_port_b, is_and? port_b_taint : port_a_taint, false, cell->src_ref());
		module->addOr(Twine{cell->name.unescape() + "_t_3_3"}, is_and? port_a_taint : port_b_taint, subexpr1, port_y_taint, false, cell->src_ref());
	}
	...
	void add_imprecise_GLIFT_logic_3(const RTLIL::Cell *cell, RTLIL::SigSpec &port_a_taint, RTLIL::SigSpec &port_b_taint, RTLIL::SigSpec &port_y_taint) {
		...
		module->addOr(Twine{cell->name.unescape() + "_t_4_1"}, port_a_taint, port_b_taint, port_y_taint, false, cell->src_ref());
	}
	...
	void add_precise_GLIFT_mux(const RTLIL::Cell *cell, RTLIL::SigSpec &port_a, RTLIL::SigSpec &port_a_taint, RTLIL::SigSpec &port_b, RTLIL::SigSpec &port_b_taint, RTLIL::SigSpec &port_s, RTLIL::SigSpec &port_s_taint, RTLIL::SigSpec &port_y_taint) {
		...
		RTLIL::SigSpec n_port_a = module->LogicNot(Twine{cell->name.unescape() + "_t_4_1"}, port_a, false, cell->src_ref());
		RTLIL::SigSpec n_port_b = module->LogicNot(Twine{cell->name.unescape() + "_t_4_2"}, port_b, false, cell->src_ref());
		RTLIL::SigSpec n_port_s = module->LogicNot(Twine{cell->name.unescape() + "_t_4_3"}, port_s, false, cell->src_ref());
		auto subexpr1 = module->And(Twine{cell->name.unescape() + "_t_4_4"}, port_s, port_a_taint, false, cell->src_ref());
		auto subexpr2 = module->And(Twine{cell->name.unescape() + "_t_4_5"}, n_port_s, port_b_taint, false, cell->src_ref());
		auto subexpr3 = module->And(Twine{cell->name.unescape() + "_t_4_6"}, n_port_a, port_b, false, cell->src_ref());
		auto subexpr4 = module->And(Twine{cell->name.unescape() + "_t_4_7"}, subexpr3, port_s_taint, false, cell->src_ref());
		auto subexpr5 = module->And(Twine{cell->name.unescape() + "_t_4_8"}, port_a, n_port_b, false, cell->src_ref());
		auto subexpr6 = module->And(Twine{cell->name.unescape() + "_t_4_9"}, subexpr5, port_s_taint, false, cell->src_ref());
		auto subexpr7 = module->And(Twine{cell->name.unescape() + "_t_4_10"}, port_a_taint, port_s_taint, false, cell->src_ref());
		auto subexpr8 = module->And(Twine{cell->name.unescape() + "_t_4_11"}, port_b_taint, port_s_taint, false, cell->src_ref());
		auto subexpr9  = module->Or(Twine{cell->name.unescape() + "_t_4_12"}, subexpr1, subexpr2, false, cell->src_ref());
		auto subexpr10 = module->Or(Twine{cell->name.unescape() + "_t_4_13"}, subexpr4, subexpr6, false, cell->src_ref());
		auto subexpr11 = module->Or(Twine{cell->name.unescape() + "_t_4_14"}, subexpr7, subexpr8, false, cell->src_ref());
		auto subexpr12 = module->Or(Twine{cell->name.unescape() + "_t_4_15"}, subexpr9, subexpr10, false, cell->src_ref());
			...
			if (cell->type.in(ID($_AND_), ID($_NAND_), ID($_OR_), ID($_NOR_))) {
				...
				if (opt_create_precise_model)
					...
					for (auto i = 1; i <= num_versions; ++i)
						taint_version.emplace_back(RTLIL::SigSpec(module->addWire(Twine{stringf("%s_y%d", cell->name.unescape().c_str(), i)}, 1)));
					...
					for (auto i = 0; meta_mux_y_ports.size() > 1; ++i) {
						for (auto j = 0; j+1 < GetSize(meta_mux_y_ports); j += 2) {
							next_meta_mux_y_ports.emplace_back(module->Mux(Twine{stringf("%s_mux_%d_%d", cell->name.unescape().c_str(), i, j)}, meta_mux_y_ports[j], meta_mux_y_ports[j+1], meta_mux_select[GetSize(meta_mux_select) - 1 - i]));
						}
