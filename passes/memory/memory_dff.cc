...
struct MemoryDffWorker
{
	...
	void handle_rd_port(Mem &mem, QuickConeSat &qcsat, int idx)
	{
		...
		log("Checking read port `%s'[%d] in module `%s': ", log_id(module, mem.memid), idx, log_id(module));
	...
	{
		auto &port = mem.rd_ports[idx];
		log("Checking read port address `%s'[%d] in module `%s': ", log_id(module, mem.memid), idx, log_id(module));
