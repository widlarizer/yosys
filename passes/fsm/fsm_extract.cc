...
static void extract_fsm(RTLIL::Wire *wire)
{
	...
	RTLIL::Cell *fsm_cell = module->addCell(stringf("$fsm$%s$%d", wire->name, module->design->twines.next_autoidx()), ID($fsm));
	...
	for (auto &cellport : cellport_list) {
		...
		RTLIL::Wire *unconn_wire = module->addWire(stringf("$fsm_unconnect$%d", module->design->twines.next_autoidx()), unconn_sig.size());
