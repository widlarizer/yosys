						...
						if (port->port_output) {
							...
							for (int j = 0; j < port->width; j++) {
								if (conn[j].wire && conn[j].wire->port_output) {
									...
									auto new_wire_name = module->uniquify(design->twines.add(stringf("$box$%s$%s$%d", cell_name_str.c_str(), port_id_str_part, j)));
		...
		for (int c = f->get(); c != EOF; c = f->get()) {
			if (c == 'M') {
				...
				for (unsigned i = 0; i < no_instances; ++i) {
					...
					Cell *instance = module->addCell(module->uniquify(design->twines.add(stringf("$sc%d", out_lit))), cell.type);
					auto out_w = module->addWire(module->uniquify(design->twines.add(stringf("$lit%d", out_lit))));
