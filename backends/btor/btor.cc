...
struct BtorWorker
{
	...
	void export_cell(Cell *cell)
	{
		...
		if (cell->is_mem_cell())
		{
			...
			if (module->design->twines.str(mem->memid)[0] == '$')
				btorf("%d state %d\n", nid, sid);
			else
				btorf("%d state %d %s\n", nid, sid, module->design->twines.unescaped_str(mem->memid));
			...
			f(f), sigmap(module), module(module), verbose(verbose), single_bad(single_bad), cover_mode(cover_mode), print_internal_names(print_internal_names), src_search(&module->design->twines), info_filename(info_filename)
	{
		...
		for (auto &mem : memories) {
			mem.narrow();
			mem_dict[module->design->twines.str(mem.memid)] = &mem;
		}
		...
		while (!ff_todo.empty() || !mem_todo.empty())
		{
			...
			for (auto &it : mtodo)
			{
				...
				btorf_push(stringf("next %s", module->design->twines.unescaped_str(mem->memid)));
				...
				btorf_pop(stringf("next %s", module->design->twines.unescaped_str(mem->memid)));
			}
		}
