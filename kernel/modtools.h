...
struct ModIndex : public RTLIL::Monitor
{
	...
	struct PortInfo {
		...
		PortInfo(RTLIL::Cell* _c, IdString _p, int _o) : cell(_c), port(_p), offset(_o) { }
	...
	void port_add(RTLIL::Cell *cell, IdString port, const RTLIL::SigSpec &sig)
	{
	...
	void port_del(RTLIL::Cell *cell, IdString port, const RTLIL::SigSpec &sig)
	{
		for (int i = 0; i < GetSize(sig); i++) {
			RTLIL::SigBit bit = sigmap(sig[i]);
			if (bit.wire) {
	...
	void notify_connect(RTLIL::Cell *cell, IdString port, const RTLIL::SigSpec &old_sig, const RTLIL::SigSpec &sig) override
	{
...
struct ModWalker
{
	...
	void add_cell_port(RTLIL::Cell *cell, IdString port, std::vector<RTLIL::SigBit> bits, bool is_output, bool is_input)
	{
